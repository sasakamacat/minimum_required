...
################################################################################
# スクリプト名: hle_script.sh
# 概要:
#   このスクリプトは、vLLMサーバーとOpenAI o3モデルを使用して、
#   HLEベンチマークでQwen3-32Bモデルを評価するためのSLURMジョブスクリプトです。
#
# 実行の流れ:
...
#--- GPU 監視 -------------------------------------------------------
nvidia-smi -i 0,1,2,3,4,5,6,7 -l 60 > ${HOME}/baseline/eval_hle/logs/nvidia-smi.log &
pid_nvsmi=$!
...
vllm serve ${HOME}/model/Qwen3-32B-SFT-GRPO \
...
echo "$HOME"
#--- 推論 -----------------------------------------------------------
python ${HOME}/baseline/eval_hle/predict.py
echo "Predict.py completed."

# predict.pyが完了した後にjudge.pyを実行します。
python ${HOME}/baseline/eval_hle/judge.py
